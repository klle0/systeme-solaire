...
class Meteorite:
    def __init__(self):
        ...
        self.taille = random.randint(3, 5)
    ...
    def cliqué(self, pos_souris):
        # Vérifie si la météorite a été cliquée
        distance = math.sqrt((self.x - pos_souris[0]) ** 2 + (self.y - pos_souris[1]) ** 2)
        return distance < self.taille
    
#pour gérer le jeu
def gérer_clic_meteorites(meteorites, pos_souris):
    global compteur_meteorites
    for meteorite in meteorites:
        if meteorite.cliqué(pos_souris): 
            meteorites.remove(meteorite)  # Retirer la météorite de la liste
            compteur_meteorites += 1  # Incrémenter le compteur
            print(f"Compteur : {compteur_meteorites}")
...
def main():
    ...
    while en_cours:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            gérer_clic_meteorites(meteorites, pos_souris)
        
        for meteorite in meteorites:
            meteorite.dessiner(window)
